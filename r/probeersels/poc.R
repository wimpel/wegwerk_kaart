...
DT <- data.table::setDT(
  readxl::read_excel("./data/test.xlsx", 
...
l <- leaflet() %>% 
  ...
  addPolylines(data = lijnen, color = ~color, weight = 5, opacity = 1,
               popup = lapply(labels.lijnen$label, HTML)) %>%
  addCircles(data = punten, color = ~color, weight = 20, opacity = 0.75,
             popup = lapply(labels.punten$label, HTML)) %>%
