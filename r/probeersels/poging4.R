...
leaflet() %>%
  ...
  onRender("function(el, x, data) {
    ...
    layergroups = {};
        for(type in data){
            var lg = L.layerGroup().addTo(this)
            layergroups[type] = lg;
...
for (group_name in names(bladen)) {
  variable_name <- bladen[group_name]
  l <- l %>%
    addPolylines(data = df[df$groep == group_name,],
                 group = group_name)
}
...
l %>%
  addLayersControl(
    ...
    baseGroups = names(bladen),
    options = layersControlOptions(collapsed = TRUE)) %>%
  hideGroup(names(bladen)) %>%
  showGroup(group_display)


###of
for (group_name in names(bladen)) {
  # plotdata <- list( lijnen = sf_geojson(df[df$groep == group_name]),
  #                   group_name = group_name)
  l <- l %>%
    onRender("function(el, x, data) {
      // maak een lege layergroup
      // plot de polylines, voeg toe aan de layergroep
      // voeg te layergroep toe aan de kaart
      layergroups = {};
      for(groep in data){
        var lg = L.layerGroup().addTo(this)  
        layergroups[groep] = lg;
      }
    }", data = sf_geojson(df))
}
